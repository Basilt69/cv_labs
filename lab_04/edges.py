...
#URL = "https://images11.esquire.ru/upload/img_cache/1bc
# /1bc2fc6d37278a78166ad6a432ded21a_ce_859x537x77x0_cropped_1332x888.jpg"
...
def description():
    task = """
        Implement the search for image boundaries by the studied methods.
        The search for boundaries can be implemented independently by operators:
            - Roberts
            - Prewitt
            - Sobel
            - Scharr
        ...
        The Canny operator is ready to use from OpenCV.
    """

    st.markdown("### Laboratory work №4")
    st.markdown("**Title:** Finding edges of an image")
...
def main():
    ...
    user_url = validate_url(
        st.text_input(f"Input image URL {FILE_TYPES}: ", URL)
    )
    ...
    if method == "1":
        thresh_lower = c1.number_input("Lower threshold:", min_value=1, max_value=99, value=50, step=1)
        thresh_upper = c2.number_input("Upper threshold:", min_value=1, max_value=199, value=150, step=1)

        res = canny(gray_image, thresh_lower, thresh_upper)
        st.image(res, width=660)

    if method == "2":
        thresh = c2.number_input("Threshold:", min_value=1, max_value=199, value=20, step=1)
    ...
    if method == "6":
        ...
        kernel_size = c2.number_input("Kernel size:", min_value=1, max_value=199, value=25, step=2)
